...
function render(data) {
    var s = data.summary;
    if (!s) {
        document.getElementById('detail').innerHTML = '<div class="pa-card text-center py-5 text-muted">No occurrences of this error in the selected range.</div>';
        return;
    }
    ...
    if (s.stack) {
        html += '<div class="pa-card mb-4">';
        html += '<h6 class="mb-3 font-heading-bold">Stack</h6>';
    ...
    html += '<div style="padding:1rem 1.25rem;border-bottom:1px solid var(--pa-border)"><h6 class="mb-0 font-heading-bold">Recent occurrences</h6></div>';
    if (!data.recent.length) {
        html += '<div class="text-center py-4 text-muted">None.</div>';
    } else {
