...
<div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-4">
    <h4 class="mb-0 font-heading-bold">{{ __('analytics.page_campaigns') }}</h4>
...
function renderCampaigns(rows) {
    if (!rows.length) {
        document.getElementById('campaigns-table').innerHTML = '<div class="text-center py-5 text-muted">{{ __("analytics.no_campaign_data") }}</div>';
        return;
    }
    ...
    html += '<th class="text-end">{{ __("analytics.col_visitors") }}</th><th class="text-end">{{ __("analytics.col_sessions") }}</th></tr></thead><tbody>';
    rows.forEach(function(row) {
        ...
        html += '<td class="text-end">' + (row.visitors || 0).toLocaleString() + '</td>';
        html += '<td class="text-end">' + (row.sessions || 0).toLocaleString() + '</td>';
        html += '</tr>';
    });
    html += '</tbody></table>';
...
function renderDrilldown(rows, source, medium, campaign) {
    ...
    if (!rows.length) {
        ...
        rows.forEach(function(row) {
            html += '<tr><td>' + (row.url || '-').replace(/^https?:\/\//, '') + '</td><td class="text-end">' + (row.visitors || 0).toLocaleString() + '</td></tr>';
        });
        html += '</tbody></table>';
    }
    document.getElementById('campaigns-table').innerHTML = html;
