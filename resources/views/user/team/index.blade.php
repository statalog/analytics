...
<p style="color:var(--pa-text-muted);max-width:720px;margin-bottom:1.5rem">
    Give other people access to your analytics. <strong>Admins</strong> can do everything you can (manage sites, goals, funnels). <strong>Viewers</strong> see reports but can't change anything.
</p>

<div class="row g-4" style="max-width:1000px">
    <div class="col-md-5">
        <div class="pa-card">
            <h6 class="mb-3" style="font-family:'Space Grotesk',sans-serif;font-weight:700">Add a team member</h6>
            <form method="POST" action="{{ route('user.team.store') }}">
                @csrf
                <div class="mb-3">
                ...
                <button type="submit" class="btn-pa-primary"><i class="bi bi-person-plus me-1"></i>Add member</button>
            </form>
        </div>
    </div>
        ...
        <div class="pa-card p-0">
            @if($members->isEmpty())
                <div class="text-center py-5">
                    ...
                    <div style="color:var(--pa-text-muted);margin-top:0.75rem">No team members yet.</div>
                </div>
            @else
                <table class="pa-table">
                    <thead>
                        <tr><th>Member</th><th>Role</th><th>Site access</th><th></th></tr>
                    </thead>
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <form method="POST" action="{{ route('user.team.update', $m) }}" class="d-inline">
                                    @csrf @method('PUT')
                            ...
                            <td class="text-end">
                                <form method="POST" action="{{ route('user.team.destroy', $m) }}" class="d-inline"
                                      onsubmit="return confirm('Remove {{ $m->user?->email }} from your team?')">
                                    @csrf @method('DELETE')
