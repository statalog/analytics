    ...
    fetch('{{ route("user.seo.sitemap.check") }}')
        ...
        .then(data => {
            ...
            if (data.issues && data.issues.length) {
                ...
                data.issues.forEach(function(i) { html += '<div class="d-flex gap-2 align-items-start mb-2"><i class="bi bi-exclamation-triangle-fill text-warning mt-1"></i><span class="text-sm">' + i + '</span></div>'; });
            }
            html += '</div>';
