...
<div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-4">
    <h4 class="mb-0 font-heading-bold">{{ __('analytics.page_custom_events') }}</h4>
...
function render(rows) {
    if (!rows.length) {
        document.getElementById('events-table').innerHTML = '<div class="text-center py-5 text-muted">{{ __("analytics.no_events") }}</div>';
        return;
    }
    var html = '<table class="pa-table"><thead><tr>';
    html += '<th>{{ __("analytics.col_event_name") }}</th>';
    html += '<th class="text-end">{{ __("analytics.col_total_occurrences") }}</th>';
    html += '<th class="text-end">{{ __("analytics.col_unique_visitors") }}</th>';
    html += '<th class="text-end">{{ __("analytics.col_first_seen") }}</th>';
    html += '<th class="text-end">{{ __("analytics.col_last_seen") }}</th>';
    ...
    rows.forEach(function(row) {
        ...
        html += '<td class="text-end">' + (row.total || 0).toLocaleString() + '</td>';
        html += '<td class="text-end">' + (row.unique_visitors || 0).toLocaleString() + '</td>';
